          ...
          Expanded(
            child: BlocBuilder<TasksBloc, TasksState>(
              builder: (context, state) {
                return TasksBloc.get(context).fetchScheduleTasksByDate().isEmpty
                    ? const NoTaskText(text: 'No tasks scheduled on this day..')
                    : ListView.builder(
                        physics: const BouncingScrollPhysics(),
                        padding: const EdgeInsets.only(bottom: 40),
                        itemCount: TasksBloc.get(context)
                            .fetchScheduleTasksByDate()
                            .length,
                        itemBuilder: (context, index) {
                          return TaskSlideFadeAnimation(
                            ...
                            child: ScheduleTask(
                              task: TasksBloc.get(context)
                                  .fetchScheduleTasksByDate()[index],
                            ),
                          );
                        },
                      );
